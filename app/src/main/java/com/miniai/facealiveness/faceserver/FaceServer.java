package com.miniai.facealiveness.faceserver;
...
import com.miniai.facealiveness.model.FaceRegisterInfo;
...
import com.miniai.imageutil.MiniAiImageFormat;
import com.miniai.imageutil.MiniAiImageUtil;
import com.miniai.imageutil.MiniAiImageUtilError;
import com.miniai.imageutil.MiniAiRotateDegree;
...
public class FaceServer {
        ...
        synchronized (this) {
            ...
            if (code != ErrorInfo.MOK) {
                ...
                try {
                    ...
                    Bitmap headBmp = getHeadImage(nv21, width, height, faceInfo.getOrient(), cropRect, MiniAiImageFormat.NV21);
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        // 创建一个头像的Bitmap，存放旋转结果图
                        Bitmap headBmp = getHeadImage(bgr24, width, height, faceInfoList.get(0).getOrient(), cropRect, MiniAiImageFormat.BGR24);
                        // 保存到本地
                        headBmp.compress(Bitmap.CompressFormat.JPEG, 100, fosImage);
                        fosImage.close();
     ...
     * @return 头像的图像数据
     */
    private Bitmap getHeadImage(byte[] originImageData, int width, int height, int orient, Rect cropRect, MiniAiImageFormat imageFormat) {
        byte[] headImageData = MiniAiImageUtil.createImageData(cropRect.width(), cropRect.height(), imageFormat);
        int cropCode = MiniAiImageUtil.cropImage(originImageData, headImageData, width, height, cropRect, imageFormat);
        if (cropCode != MiniAiImageUtilError.CODE_SUCCESS) {
        ...
        MiniAiRotateDegree rotateDegree = null;
        switch (orient) {
            case FaceEngine.ASF_OC_90:
                rotateDegree = MiniAiRotateDegree.DEGREE_270;
                break;
            case FaceEngine.ASF_OC_180:
                rotateDegree = MiniAiRotateDegree.DEGREE_180;
                break;
            case FaceEngine.ASF_OC_270:
                rotateDegree = MiniAiRotateDegree.DEGREE_90;
                break;
        ...
        if (rotateDegree != null){
            ...
            rotateCode = MiniAiImageUtil.rotateImage(headImageData, rotateHeadImageData, cropRect.width(), cropRect.height(), rotateDegree, imageFormat);
            if (rotateCode != MiniAiImageUtilError.CODE_SUCCESS) {
        ...
        if (MiniAiImageUtil.imageDataToBitmap(rotateHeadImageData, headBmp, imageFormat) != MiniAiImageUtilError.CODE_SUCCESS) {
